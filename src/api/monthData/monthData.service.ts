...
async function getByMonth({ user, year, month }: { user: User; year: number; month: number }) {
  try {
...
function _getDemoData(): NewYearData[] {
  return [
    {
...
function _getYearDataCollection() {
  return dbService.getCollection<YearData>('yearData')
}
